...
#include <CBUSESP8266.h>            // ESP8266 CAN bus library
...
// module name
unsigned char mname[7] = { 'E', 'M', 'P', '8', '2', '6', '6' };
...
void setup() {
  ...
  Serial.begin (115200);
  Serial << endl << endl << F("> ** CBUS empty module for ESP8266 v1 ** ") << __FILE__ << endl;
  ...
  CBUS.setNumBuffers(20);         // more buffers = more memory used, fewer = less
  CBUS.setPins(6, 7);             // select ESP8266 GPIO pins for the CAN controller chip select and interrupt signals

  byte cret;

  if ((cret = CBUS.begin())) {
    Serial << F("> CAN bus started ok") << endl;
  } else {
    Serial << F("> error starting CAN bus, ret = ") << cret << endl;
  }
...
void printConfig(void) {
  ...
  // copyright
  Serial << F("> © Duncan Greenwood (MERG M5767) 2020") << endl;
  return;
}
